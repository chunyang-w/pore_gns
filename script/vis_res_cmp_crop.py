...
# crop the cube
left_idx = int(left_bd * cube_pred.shape[1])
right_idx = int(right_bd * cube_pred.shape[1])

front_idx = int(front_bd * cube_pred.shape[2])
back_idx = int(back_bd * cube_pred.shape[2])

top_idx = int(top_bd * cube_pred.shape[3])
bottom_idx = int(bottom_bd * cube_pred.shape[3])


cube_pred = cube_pred[
    :, left_idx:right_idx, front_idx:back_idx, bottom_idx:top_idx
]  # noqa: E501
cube_gt = cube_gt[
    :, left_idx:right_idx, front_idx:back_idx, bottom_idx:top_idx
]  # noqa: E501
...
def update_slice(t):
    ...
    print(f"dice score at frame {t}: {dice_list[t]:.4f}")
