...
class PointEmbed(nn.Module):
    def __init__(self, hidden_dim=48, dim=128):
        ...
        e = (
            torch.pow(
                2,
                torch.arange(self.embedding_dim // 6),
            ).float()
            * np.pi
        )
        e = torch.stack(
...
class AutoEncoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        get_latent_attn = lambda: PreNorm(  # noqa E731
            dim,
            Attention(
                dim,
                heads=heads,
        ...
        get_latent_ff = lambda: PreNorm(  # noqa E731
            dim,
            FeedForward(dim, drop_path_rate=0.1),
        )
...
def ae_d512_m1024(N=2048):
    return create_autoencoder(dim=512, M=1024, N=N, determinisitc=True)
...
def ae_d256_m256(N=2048):
    return create_autoencoder(dim=256, M=256, N=N, determinisitc=True)
